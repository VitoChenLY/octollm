...
func TestApiChatCompletionsToApiMessages_convertRequestBody_SimpleText(t *testing.T) {
	converter := NewApiChatCompletionsToApiMessages(nil)
	ctx := context.Background()

	// Create Anthropic request
	text := "Hello, Claude!"
	anthropicReq := &anthropic.ApiMessagesRequest{
	...
	// Create UnifiedBody
	srcBody := octollm.NewBodyFromBytes([]byte{}, &octollm.JSONParser[anthropic.ApiMessagesRequest]{})
	...
	openaiReq, ok := parsed.(*openai.ApiChatCompletionsRequest)
	require.True(t, ok, "parsed body should be *openai.ApiChatCompletionsRequest")
...
func TestApiChatCompletionsToApiMessages_convertNonStreamResponseBody(t *testing.T) {
	...
	openaiResp := &openai.ApiChatCompletionsResponse{
		Id:     "msg_123",
		Model:  "gpt-4",
	...
	srcBody := octollm.NewBodyFromBytes([]byte{}, &octollm.JSONParser[openai.ApiChatCompletionsResponse]{})
	...
	claudeResp, ok := parsed.(*anthropic.ApiMessagesResponse)
	require.True(t, ok, "parsed body should be *anthropic.ApiMessagesResponse")
